 ...
 */
export class Pulsating implements Imager {
  ...
  constructor() {
    this.imager = new CompositeImager(
      ...
      .addImager(
        new TextImager('XMAS', 24, 'monospace', 'rgb(255, 0, 0)'),
        Position.center()
      );
  }
  ...
  getMatrix(frame: number, previousMatrix: Matrix | null): Matrix {
    const matrix = this.imager.getMatrix(frame, previousMatrix);
    return new Matrix(Dimensions.square(64),(x,y)=>{
      return { color: matrix.get(x,y).color,
               brightness: Math.floor((Math.sin((frame / 100) * 2 * Math.PI + (x + y) / 8) + 1) / 2 * 255) };
    } );
  }
}

// Self-register this scene
SceneRegistry.register(() => new Pulsating());
